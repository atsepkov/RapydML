...
def get_attr(tag):
	...
	if attr_string == '':
		...
		for i in range(len(final_attr_list)):
			...
			if final_attr_list[i][0] in ('"', "'") and final_attr_list[i].find('=') != -1:
				# strip quotes from attribute name
...
class TemplateEngine:
	"""
	Helper class for generating HTML templates used by various templating engines such as Django, web2py, or Rails
	"""
...
class Parser:
	...
	def close_last_element(self):
		...
		if self.last_opened_element not in self.special_tags and re.search('^%s</%s>' % (self.indent_marker*self.indent, self.last_opened_element), tag):
			self.write(' />\n', -2)
		else:
			self.write(tag)
	...
	def create_method(self, line):
		indent = self.find_indent(line) #this should always be zero
		if line[:4] == 'def ': # this prevents us from overlooking consecutively declared methods
			...
			if indent == 0:
				...
				self.method_map[self.creating_method].add_line(line[len(self.indent_marker):])
		return False
	...
	def unroll_loop(self):
		...
		for arg in loop_array:
			self.handle_line('%s%s(%s)' % (self.indent_marker*loop_indent, loop_name, arg))
	...
	def create_loop(self, line):
		...
		if tag[:4] == 'for ':	# new loop started (either within old loop, or outside)
			self.handle_indent(indent-self.indent, None)
			self.indent = indent
			...
			if indent_diff < 1:
				...
				self.indent = indent
				self.method_map[loop_name].add_line(line[len(self.indent_marker*(loop_indent+1)):])
	...
	def parse_template_engine_call(self, line, indent):
		...
		if compressed.find('create(') != -1:
			...
			try:
				...
				whitespace = self.indent_marker*indent
				...
				if not self.template_engines[method_pair[0]].is_submethod(method_pair[1], indent):
					self.handle_indent(indent_diff, whitespace + self.template_engines[method_pair[0]].end_method(method_pair[1]))
				else:
					#note the indent_diff+1, we want to close inner logic, but not the method itself
					endtag = self.template_engines[method_pair[0]].end_method(method_pair[1], False)
					if endtag is None:
						self.handle_indent(indent_diff+1, None)
						self.element_stack.pop()
					else:
						self.handle_indent(indent_diff+1, whitespace + endtag)
				attr = get_attr(line)
	...
	def check_for_algebra(self, tag):
	...
	def handle_indent(self, indent_diff, endtag):
		# push to or pop from the stack, depending on indent
		if indent_diff < 1 and self.element_stack: #pop
			while indent_diff < 1:
				self.close_last_element()
				indent_diff += 1
		elif indent_diff > 1:
			raise ParserError('Incorrect indentation')
		self.element_stack.append(endtag)
		# TODO: change last opened element as well?
		# TODO: set self.indent here as well?
	...
	def handle_verbatim_call(self, line):
		...
		if self.current_verbatim is None:
			...
			self.verbatim_indent = self.find_indent(line)
			if self.verbatim[self.current_verbatim][0] != '':
				self.verbatim_buffer += self.indent_marker*self.verbatim_indent + self.verbatim[self.current_verbatim][0]
			self.last_opened_element = None
			...
			if indent > self.verbatim_indent:
				...
				if self.verbatim[self.current_verbatim][0] == '' and line.find(self.indent_marker) == 0:
					line = line[len(self.indent_marker):]
				
				self.verbatim_buffer += line
			else:
				# end of verbatim logic
				if self.verbatim[self.current_verbatim][1] != '':
					self.verbatim_buffer += self.indent_marker*self.verbatim_indent + self.verbatim[self.current_verbatim][1]
	...
	def handle_line(self, line):
		...
		if element in self.method_map.keys():
			self.handle_indent(indent_diff, None)
			self.indent = indent
		...
		self.handle_indent(indent_diff, htmlend)
		...
		with open(filename, 'r') as source:
			...
			for line in source:
				...
				try:
					...
					print "Error in %s: %d: %s" % (filename, line_num, error.message)
					print line
					sys.exit()
				except:
					# on all other errors
					print "Error in %s: %d: %s" % (filename, line_num, "'%s' caused the following uncaught exception:" % line.strip())
					print line
					raise
